...
- **服屋を探す** — ホームから Google マップで周辺の服屋を検索します
...
| `npm run test:contrast:landscape` | 同上を横向きで |
| `npm run test:landscape` | 横向きでレイアウトが破綻していないかの検証 |
...
縦向き前提で作っていたため、**横向きにすると下部のボタンが画面の外に出ていました。**

横画面では枠の高さが 344〜374px まで潰れるのに、中身は縦向きのままで 537px ありました。さらに悪いことに、ガチャボタンは横画面のほうが**大きく**なっていました。`sm:` が幅 640px 以上で効くためで、**制約が高さなのにサイズを幅で決めていた**のが原因です。

| 画面 | 枠 | 中身 | はみ出し | ガチャ |
| --- | --- | --- | --- | --- |
| iPhone 縦 390×844 | 390×844 | 844px | 0 | 192px |
| iPhone 横 844×390 | 384×374 | 537px | **163px** | **208px** |
| 小型機 横 740×360 | 384×344 | 537px | **193px** | **208px** |

高さで分岐する変種を `src/styles.css` に定義し、横画面では要素を詰めています。
...
| 要素 | 縦向き | 横向き（short:） |
| --- | --- | --- |
| ガチャボタン | 192px（`sm:` で 208px） | 88px |
| 「TAP TO SPIN」「自動提案します」 | 表示 | 非表示 |
| 下部ボタンのアイコン | 48px | 32px |
| インストール案内 | 70px | 58px（アイコンを省く） |

インストール案内が出ると 70px ぶん押し下げられて、それだけで下部ボタンが画面外に出ていました。案内側も縮めてあります。

`npm run test:landscape` が2つの横画面サイズで、主要な操作が枠内にあること、ホームがスクロールせずに収まること、インストール案内が出ても隠れないこと、タップ対象が 44px 以上あること、縦向きが従来どおりであることを検証します。`npm run test:contrast:landscape` は同じ画面をコントラスト比でも見ます（要素を縮めたので、文字の大きさで判定の閾値が変わるため）。

## 服屋を探す

ホームの「服屋を探す」を押すと、**Google マップが周辺の服屋を検索した状態で開きます。** 間に画面を挟みません。提案結果の画面にも同じリンクがあります。

```
現在地あり: https://www.google.com/maps/search/服屋/@33.9189,133.1818,14z
現在地なし: https://www.google.com/maps/search/?api=1&query=服屋
```

中心を指定できるのは前者の形式だけで、公式ドキュメントには載っていません（マップ自身が生成する URL と同じ形です）。後者は公式の Maps URLs API です。

リンク先は `index.html` に直接書いてあるので、JS が動かなくてもリンクとして成立します。天気の「現在地を使う」で位置が分かったときだけ、`src/ui/find-shops.js` が `href` に中心を足します。ボタン＋`window.open` にしていないのは、位置情報の取得を挟んでから開くと操作との紐づきが切れ、ポップアップブロックに当たることがあるためです。

### ショップ画面をやめた経緯

以前は店舗一覧の画面があり、`src/data/shops.js` に2店舗（minami・キャマラド）を手で登録していました。次の理由で畳みました。

| 出していたもの | 問題 |
| --- | --- |
| 営業時間 | 手入力。変わっても誰も気づかない |
| 営業中バッジ | その手入力から計算していたので、**自信を持って間違える** |
| 距離 | 座標が `null` のまま一度も表示されたことがない |
| 商品の写真・名前・価格 | ストックフォトと創作。削除後は空欄 |

どれも Google マップ側により正確な情報があります。あわせて、提案結果の画面にあった「サコッシュバッグ / 近くの店舗で取扱中」も削除しました。在庫を確かめる手段が無いまま取扱中と書いていたためです。

商品写真の仕組み（`npm run shop:photos` と `tests/shop-photos.mjs`）、Haversine 式の距離計算（`distanceKm` / `formatDistance`）も、呼び出し元が消えたので削除しています。

### Places API を使っていない理由

アプリ内に検索結果を並べる方が親切ですが、見送っています。

| 制約 | 内容 |
| --- | --- |
| APIキーの露出 | クライアントアプリなのでバンドルに載る。リファラ制限は `Referer` を偽装されうるので、実効的な歯止めは Cloud Console の日次クォータ上限 |
| 請求先アカウント | 無料枠の範囲で使う場合でもクレジットカードの登録が要る（Firebase は Spark プランのまま運用中） |
| ToS | Place ID 以外のコンテンツは原則キャッシュ不可。Firestore に溜める設計が取れず、開くたびに呼ぶことになる |

リンクなら、このどれにも当たりません。導入する場合、`connect-src` は既に `https://*.googleapis.com` を許可しているので `places.googleapis.com` は CSP の変更なしで通ります（Maps JavaScript API を使う場合は `script-src` の追加が必要）。
...
src/
  ...
  domain/
    ...
    dates.js              日付ユーティリティ
    geo.js                Google マップ検索の URL を組み立てる
  ui/                   画面ごとの描画とフォーカス管理
    pwa.js              インストール導線・更新通知・オフライン表示
    settings.js         設定画面（データの保存先）
    find-shops.js       「服屋を探す」のリンク先を現在地に合わせる
