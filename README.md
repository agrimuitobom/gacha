...
- **周辺の服屋** — 近隣ショップの営業時間・距離・商品（商品は実物の写真のみ。未登録のうちは「準備中」と表示）。ほかの店は Google マップで検索できます
...
載せている店舗は手で登録した数軒だけなので、それ以外を探す導線として **Google マップへのリンク**を置いています。現在地が分かっていれば地図の中心に指定し、分からなければ中心を指定せずに開いてマップ側の位置情報に任せます。
...
中心を指定できるのは前者の形式だけで、これは公式ドキュメントには載っていません（マップ自身が生成する URL と同じ形です）。後者は公式の Maps URLs API です。

### なぜ Places API を使わないのか

アプリ内に検索結果を並べる方が親切ですが、いまは見送っています。
...
| ToS | Place ID 以外のコンテンツは原則キャッシュ不可。Firestore に店舗データを溜める設計が取れず、開くたびに呼ぶことになる |

リンクなら、このどれにも当たりません。

導入するとしたら、`connect-src` は既に `https://*.googleapis.com` を許可しているので **`places.googleapis.com` は CSP の変更なしで通ります**（Maps JavaScript API を使う場合は `script-src` の追加が必要）。手で登録した店舗は Google マップに載っていない可能性があるため、置き換えではなく併記になります。

### 現在地の扱い

現在地は距離表示とマップ検索の中心の両方に効くので、分からないうちは取得の導線を出し、分かったら引っ込めます。以前は座標を持つ店舗が1つも無いと導線自体を出していませんでしたが、マップ検索の中心として使えるようになったため、常に出すようにしました。
